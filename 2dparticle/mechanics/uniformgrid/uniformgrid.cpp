...
void plotter(SpatialGrid &grid, Texture2D &circleTex, float xRange,
  ...
  for (auto &client : grid.m_clients) {
    ...
    if (client.m_shape == Shape::ball) {
      ...
      //   Vector2 velocity_normal{
      //       Vector2Scale(Vector2Normalize(client.m_velocity),
      //       PARTICLE_RADIUS)};
      //   Vector2 projectedVelocityNormal{
      //       projectedVector(velocity_normal, xRange)};
      //   Vector2 test{Vector2Add(projectedClientpos,
      //   projectedVelocityNormal)}; DrawLineEx(projectedClientpos, test, 1.0f,
      //   GREEN); client.m_collided = false;
      // }
